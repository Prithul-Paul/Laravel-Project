...
<div class="invoice-area">
    <div class="invoice-wrapper">
        <div class="invoice-header">
            <h1 class="invoice-title" style="text-align:center;">Invoice Heder</h1>
        </div>
            ...
            <div class="invoice-details-flex">
                <div class="invoice-single-details">
                    ...
                    <ul class="details-list">
                        <li class="list">{{$result->name}}</li>
                        <li class="list"> <a href="#">{{$result->email}} </a> </li>
                        <li class="list"> <a href="#"> {{$result->mobile}}</a> </li>
                    </ul>
                </div>
                <div class="invoice-single-details">
                    <h4 class="invoice-details-title">Ship To:</h4>
                    <ul class="details-list">
                        <li class="list"> <strong>City: </strong> {{$result->city}}</li>
                        <li class="list"> <strong>State: </strong>{{$result->state}}</li>
                        <li class="list"> <strong>Pincode: </strong>{{$result->pincode}}</li>
                    </ul>
                </div>
            </div>
        </div>
        ...
        <div class="item-description">
            ...
            <table class="custom--table">
                ...
                @foreach($result->order_details as $item)
                        ...
                        @endphp
                <tr>
                    <td>{{$item->product_attr_with_size_color_product->product->product_name}}, Size: {{$item->product_attr_with_size_color_product->size->size}}, Color: {{$item->product_attr_with_size_color_product->color->color}}</td>
                    <td>Rs. {{$item->price}}</td>
                    <td>{{$item->qty}}</td>
